...
### 🏆 最佳模型
**Qwen3-Reranker-0.6B (qwen3_template, max_len=768)** 在所有核心指标上显著超越 BGE-Reranker-v2-m3
...
### 2.4 统计显著性检验 (基于 50 queries)

**Finetuned 模型对比**:

| 对比 | MRR 差异 | 显著性 | 说明 |
|------|----------|--------|------|
| Qwen3-768 vs BGE-768 | +0.0662 (+9.3%) | ✅ 显著 | Qwen3 在 50 queries 上稳定优于 BGE |
| Qwen3-768 vs BGE-512 | +0.0655 (+9.2%) | ✅ 显著 | 同上 |
| BGE-768 vs BGE-512 | -0.0007 (-0.1%) | ❌ 不显著 | MRR 无显著差异 |

**NDCG@10 对比**:

| 对比 | NDCG 差异 | 显著性 | 说明 |
|------|----------|--------|------|
| Qwen3-768 vs BGE-768 | +0.0239 (+2.8%) | ✅ 显著 | Qwen3 排序质量更优 |
| Qwen3-768 vs BGE-512 | +0.0479 (+5.8%) | ✅ 显著 | 768 长度优势明显 |
| BGE-768 vs BGE-512 | +0.0240 (+2.9%) | ⚠️ 边界显著 | 768 在 NDCG 上小幅优于 512 |

**注**: 显著性基于 50 queries 的人工标注评测，p<0.05 视为显著。
...
1. **MRR@10 几乎无差异**
   - Finetuned: 0.7103 vs 0.7096 (差距仅 -0.07%，不显著)
   - Base: 512 甚至略优于 768 (0.6553 vs 0.6443)

2. **NDCG@10 有小幅提升**
   - Finetuned: 0.8282 → 0.8522 (+2.9%，边界显著)
   - 说明更长的序列对排序质量略有帮助
...
| **NDCG 优先** | BGE-768 | NDCG 提升显著，排序质量更优 |
...
|---------|-----------|---------------|---------------|--------|
| **Qwen3-768** | 10/50 | -0.106 | -0.500 | 20.0% |
| **BGE-768** | 10/50 | -0.091 | -0.581 | 20.0% |
| **BGE-512** | 10/50* | -0.091* | -0.581* | 20.0%* |

*BGE-512 数据基于 2026-01-12 训练

**关键观察**:
- 所有模型的退化率均为 20% (10/50 queries)
- Qwen3 的中位降幅较小 (-0.106 vs BGE-0.091)，表明退化更温和
- BGE 出现极端退化案例 (-0.581)，Qwen3 最大降幅为 -0.500
...
**修订记录**:
- v2.0 (2026-01-18): 结构优化，新增执行摘要、退化案例分析、统计显著性检验
...
**评测输出**:
- Prefix: `bge_m3_20260117` (复用 768 的评测结果)
- 注: BGE-512 使用与 BGE-768 相同的评测输出文件
...
### D.1 配对 t 检验 (Paired t-test)

**目的**: 检验两个模型在同一查询集上的性能差异是否显著

**假设**:
- H0 (零假设): 两个模型的 MRR 均值无显著差异
- H1 (备择假设): 两个模型的 MRR 均值有显著差异

**计算**:
```python
from scipy import stats

t_stat, p_value = stats.ttest_rel(model_a_mrrs, model_b_mrrs)
...
**定义**: 0.05 < p < 0.10，表示有显著趋势但证据不足

**处理**: 需要更多数据或更精细的实验设计来验证
